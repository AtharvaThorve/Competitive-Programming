...
public class P {
	...
	public static void main(String[] args) throws IOException {
		...
		String x = fr.next().trim();
		String op = fr.next().trim();
		String y = fr.next().trim();
		fr.next().trim();
		String z = fr.next().trim();

		// Comparing x and y
		for(int i = 0; i < x.length(); ++i) {
			String firstX = x.substring(0, i);
			String secondX = x.substring(i);
			for(int j = 0; j < y.length(); ++j) {
				String firstY = y.substring(0, j);
				String secondY = y.substring(j);
				String sx = firstY + secondX;
				String sy = firstX + secondY;
				long nx = Long.parseLong(sx);
				long ny = Long.parseLong(sy);
				if(op.equals("+")) {
					if(nx + ny == Long.parseLong(z)) {
						System.out.println(sx + " " + op + " " + sy + " = " + z);
						return;
					}
				} else if(op.equals("*")) {
					if(nx * ny == Long.parseLong(z)) {
		...
		for(int i = 0; i < x.length(); ++i) {
			...
			for(int j = 0; j < z.length(); ++j) {
				String firstZ= z.substring(0, j);
				String secondZ = z.substring(j);
				String sx = firstZ + secondX;
				String sz = firstX + secondZ;
				long nx = Long.parseLong(sx);
				long nz = Long.parseLong(sz);
				if(op.equals("+")) {
					if(nx + Long.parseLong(y) == nz) {
						System.out.println(sx + " " + op + " " + y + " = " + sz);
						return;
					}
				} else if(op.equals("*")) {
					if(nx * Long.parseLong(y) == nz) {
						System.out.println(sx + " " + op + " " + y + " = " + nz);
						return;
					}
				}
			}
		...
		for(int i = 0; i < y.length(); ++i) {
			String firstY = y.substring(0, i);
			String secondY = y.substring(i);
			for(int j = 0; j < z.length(); ++j) {
				String firstZ= z.substring(0, j);
				String secondZ = z.substring(j);
				String sy = firstZ + secondY;
				String sz = firstY + secondZ;
				long ny = Long.parseLong(sy);
				long nz = Long.parseLong(sz);
				if(op.equals("+")) {
					if(Long.parseLong(x) + ny == nz) {
						System.out.println(x + " " + op + " " + sy + " = " + sz);
						return;
					}
				} else if(op.equals("*")) {
					if(Long.parseLong(x) * ny == nz) {
						System.out.println(x + " " + op + " " + sy + " = " + nz);
						fw.close();
						return;
					}
				}
