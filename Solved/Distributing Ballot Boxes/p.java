...
public class p {
	...
	public static void main(String[] args) throws IOException {
		...
		while (true) {
			...
			for (int i = 0; i < n; ++i) {
				arr[i] = fs.nextInt();
				pq.add(new Custom((double) arr[i], 1d, arr[i]));
			}
			b -= n;

			while (b-- > 0) {
				Custom c = pq.poll();
				c = new Custom(Math.ceil(c.getThird() / (c.getSecond() + 1)), c.getSecond() + 1, c.getThird());
				pq.add(c);
			}
			bw.write(pq.peek().getFirst().intValue() + "\n");
			fs.nextLine();
		}
...
class Custom {
	...
	public Custom(Double first, Double second, Integer third) {
		this.first = first;
		this.second = second;
		this.third = third;
	}
