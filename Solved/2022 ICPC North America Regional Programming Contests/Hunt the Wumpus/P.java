...
public class P {
	...
	public static void main(String[] args) throws IOException {
		...
		while(true) {
			...
			for(int i = 0; i < 4; ++i) {
				// fw.println(Arrays.toString(wumpus[i]) + " " + i);
				if(wumpus[i][0] == -1 && wumpus[i][1] == -1)
					continue;

				if(x == wumpus[i][0] && y == wumpus[i][1]) {
					wumpus[i][0] = -1;
					wumpus[i][1] = -1;
					fw.println("You hit a wumpus!");
					continue;
				}

				minDist = Helper.min(minDist, Math.abs(x - wumpus[i][0]) + Math.abs(y - wumpus[i][1]));
				// fw.println(minDist + " " + wumpus[i][0] + " " + wumpus[i][1]);
			}
		...
		fw.println("Your score is " + count + " moves.");
