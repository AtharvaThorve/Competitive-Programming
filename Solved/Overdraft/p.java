...
public class p {
	...
	public static void main(String[] args) throws IOException {
		...
		int qt = Integer.parseInt(br.readLine().trim());
		long total = 0;
		long ans = 0;
		while (qt-- > 0) {
			...
			total += t;
			if(total < 0)
				ans = Math.max(Math.abs(total), ans);
		}
		// total = total < 0 ? Math.abs(total) : 0;
		bw.write(ans + "");
		bw.flush();
		bw.close();
	}
...
class Helper {
	...
	/*
	 * Returns the index of the first element in the array which has a value not less than the key
	 */
