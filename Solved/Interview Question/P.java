...
public class P {
	...
	public static void main(String[] args) throws IOException {
		...
		int a = helper(fizz, list, c, d, "Fizz");
		int b = helper(buzz, list, c, d, "Buzz");
	...
	private static int helper(int val, List<String> list, int c, int d, String x) {
		int a = 0;
		if(val >= 2) {
			int idx = -1;
			for(int i = 0; i < list.size(); ++i) {
				if(list.get(i).equals(x) || list.get(i).equals("FizzBuzz")) {
					if(idx == -1) {
						idx = i;
					} else {
						a = i - idx;
