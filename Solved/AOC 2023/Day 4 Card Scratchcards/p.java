...
public class p {
	...
	public static void main(String[] args) throws IOException {
		...
		while (qt-- > 0) {
			...
			sarr[n-qt-1][0] = s[0];
			sarr[n-qt-1][1] = s[1];
			countMap.put((long)n-qt-1, 1l);
		}
		for(int i = 0; i < sarr.length; ++i) {
			String[] s = sarr[i];
			HashSet<String> set1 = new HashSet<>();
			HashSet<String> set2 = new HashSet<>();
			for(String x: s[0].trim().split(" ")) {
				x = x.trim();
				if(x.length() > 0)
					set1.add(x);
			}
			for(String x: s[1].trim().split(" ")) {
				x = x.trim();
				if(x.length() > 0)
					set2.add(x);
			}
			HashSet<String> commonValues = new HashSet<>(set1);
        	commonValues.retainAll(set2);
			int count = commonValues.size();
			for(int k = 0; k < countMap.get((long)i); ++k) {
				for(long j = i+1; j <= count+i; ++j) {
					long temp = countMap.get(j) + 1;
					countMap.put(j, temp);
				}
			}
		}
