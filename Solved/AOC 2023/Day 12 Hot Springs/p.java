...
class p {
	...
	public static void main(String[] args) throws IOException {
		...
		while (qt-- > 0) {
			String s1 = fs.next();
			s1 = s1 + "?" + s1 + "?" + s1 + "?" + s1 + "?" + s1;
			String[] s2 = fs.next().split(",");
			int[] vals = new int[s2.length * 5];
			for (int j = 0; j < 5; ++j) {
				for (int i = 0; i < s2.length; ++i) {
					vals[j * s2.length + i] = Integer.parseInt(s2[i]);
				}
			}
			ans += solve(s1, vals);
		}
	...
	private static int solve(String s, int[] vals) {
		if (s.equals("")) {
			return vals.length == 0 ? 1 : 0;
		}
		...
		if (s.charAt(0) == '.' || s.charAt(0) == '?') {
			result += solve(s.length() > 1 ? s.substring(1) : "", vals);
		}

		if (s.charAt(0) == '#' || s.charAt(0) == '?') {
			if (vals[0] <= s.length() && !s.substring(0, vals[0]).contains(".")
					&& (vals[0] == s.length() || s.charAt(vals[0]) != '#')) {
				result += solve(s.length() > vals[0] + 1 ? s.substring(vals[0]+1) : "", Arrays.copyOfRange(vals, 1, vals.length));
			}
		}
