...
public class p {
	...
	public static void main(String[] args) throws IOException {
		...
		while (qt-- > 0) {
			...
			String[] sarr = fs.nextLine().trim().split(" ");
			for(String s: sarr) {
			...
			while(true) {
				...
				ArrayList<Integer> currRow = rows.get(rows.size()-1);
				for(int i = 1; i < currRow.size(); ++i) {
					diff.add(currRow.get(i) - currRow.get(i-1));
				}
				rows.add(diff);
				if(checkAllSame(diff)) {
					break;
				}
			}
			
			for(int i = rows.size() - 1; i > 0; --i) {
				ArrayList<Integer> currRow = rows.get(i);
				ArrayList<Integer> prevRow = rows.get(i-1);
				int x = prevRow.get(prevRow.size()-1);
				int y = prevRow.get(0);
				x += currRow.get(currRow.size() - 1);
				y -= currRow.get(0);
				prevRow.add(x);
				prevRow.add(0, y);
			}
			...
			// }
			// ans += rows.get(0).get(rows.get(0).size()-1);
			ans += rows.get(0).get(0);
		}
	...
	private static boolean checkAllSame(ArrayList<Integer> arrayList) {
		int first = arrayList.get(0);
        for (int element : arrayList) {
            if (element != first) {
                return false; // Not all elements are 0
            }
        }
