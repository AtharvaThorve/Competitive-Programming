...
public class Prime_Finder {
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < qt; i++) {
            ...
            if (i != qt - 1) {
                System.out.println(); // Separate test cases by an empty line except for the last one
            }
		}
    ...
    public static void printPrimesInRange(int m, int n) {
        boolean[] isPrime = new boolean[n - m + 1]; // Adjusted array size to cover the range [m, n]
        ...
        for (int p = 2; p * p <= n; p++) {
            for (int i = Math.max(p * p, (m + p - 1) / p * p); i <= n; i += p) {
                if (i >= m && i != p) { // Exclude p itself if it falls within the range [m, n]
                    isPrime[i - m] = false;
                }
            }
        }
