...
public class p {
	...
	public static void main(String[] args) throws IOException {
		...
		while (true) {
			...
			if (n == 0 && m == 0 && q == 0 && s == 0)
				break;
			ArrayList<PriorityQueue<Pair<Integer, Integer>>> adjl = new ArrayList<>();
			// int[][] graph = new int[n][n];
			// ArrayList<ArrayList<Integer>> indices = new ArrayList<>();
			for (int i = 0; i < n; ++i) {
				adjl.add(new PriorityQueue<>(new Comparator<Pair<Integer, Integer>>() {
					@Override
	...
	private static int[] path(int src, int[][] graph, ArrayList<ArrayList<Integer>> indices) {
		...
		while (!pq.isEmpty()) {
			...
			visited[p.getFirst()] = true;
			visitedCount++;
			if (visitedCount == graph.length)
				break;
			for (int i: indices.get(p.getFirst())) {
				if (!visited[i] && graph[p.getFirst()][i] + p.getSecond() < dist[i]) {
					dist[i] = graph[p.getFirst()][i] + p.getSecond();
	...
	private static int[] path(int src, ArrayList<PriorityQueue<Pair<Integer, Integer>>> adjl) {
		...
		while (!pq.isEmpty()) {
			...
			visited[p.getFirst()] = true;
			visitedCount++;
			if (visitedCount == adjl.size())
				break;
			for (Pair<Integer, Integer> x: adjl.get(p.getFirst())) {
				if (!visited[x.getFirst()] && x.getSecond() + p.getSecond() < dist[x.getFirst()]) {
					dist[x.getFirst()] = x.getSecond() + p.getSecond();
					pq.add(new Pair<>(x.getFirst(), dist[x.getFirst()]));
				}
			}
		}
		return dist;
...
class Helper {
	...
	// bfs of graph
	static ArrayList<String> getbfs(int s, ArrayList<ArrayList<Integer>> list, boolean vis[]) {
