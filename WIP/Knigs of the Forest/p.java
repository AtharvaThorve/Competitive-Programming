...
public class p {
	...
	public static void main(String[] args) throws IOException {
		...
		int karlYear = fs.nextInt();
		int karlStrength = fs.nextInt();
		...
		for (int i = 0; i < n + k - 2; ++i) {
			...
			pq = map.getOrDefault(year, new PriorityQueue<>(Comparator.reverseOrder()));
			pq.add(strength);
		}
		...
		for (Map.Entry<Integer, PriorityQueue<Integer>> entry : map.entrySet()) {
			while (!pq.isEmpty()) {
				entry.getValue().add(pq.poll());
			}
			if (entry.getValue().poll() == karlStrength) {
				bw.write(entry.getKey() + "");
				flag = true;
				break;
			}
			while (!entry.getValue().isEmpty()) {
				pq.add(entry.getValue().poll());
			}
		}
		if (!flag) {
			bw.write("unknown");
